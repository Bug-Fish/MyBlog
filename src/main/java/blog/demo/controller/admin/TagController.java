...
public class TagController {
    ...
    @GetMapping("/tags")
    public String tag(@PageableDefault(size = 4, sort = {"id"}, direction = Sort.Direction.DESC) Pageable pageable,
                      Model model) {
    ...
    public String post(@Valid Tag tag, BindingResult result, RedirectAttributes attributes) {
        ...
        if (tag1 != null) {
            result.rejectValue("name", "nameError", "该标签不能重复添加哦");
        }
        if (result.hasErrors()) {
            return "admin/tags_input";
        }
    ...
    public String editPost(@Valid Tag tag, BindingResult result, @PathVariable Long id, RedirectAttributes attributes) {
        ...
        if (type1 != null) {
            result.rejectValue("name", "nameError", "该分类不能重复添加哦");
        }
        if (result.hasErrors()) {
            return "admin/tags_input";
        }
